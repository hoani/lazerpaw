...
class Cat:
    ...
    def draw(self, img):
        ...
        b, g, r, mask = cv.split(
            self.img.copy()[
                ymin - yoffset : ymax - yoffset, xmin - xoffset : xmax - xoffset
            ]
        )
...
class Simulation:
    ...
    def update(self):
        ...
        if self.lazerOn:
            ...
            lazerx = (invMatrix[0, 0] * lx + invMatrix[0, 1] * ly + invMatrix[0, 2]) / (
                invMatrix[2, 0] * lx + invMatrix[2, 1] * ly + invMatrix[2, 2]
            )
            lazery = (invMatrix[1, 0] * lx + invMatrix[1, 1] * ly + invMatrix[1, 2]) / (
                invMatrix[2, 0] * lx + invMatrix[2, 1] * ly + invMatrix[2, 2]
            )
